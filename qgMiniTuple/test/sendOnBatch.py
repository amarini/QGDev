...
for idx0,fl in enumerate(fileChunks):
	...
	if opts.put != "":
		...
		print >> sh, '[ "${EXIT}" == "0" ] && { ' + EOS + " mkdir " + "/eos/cms"+opts.put +'  ; cmsStage -f ${WORKDIR}/qgMiniTuple.root ' + opts.put + '/qgMiniTuple_%(idx)d.root  && touch sub_%(idx)d.done || echo "cmsStage fail" > sub_%(idx)d.fail; }'%{'idx':idx}
	else:
