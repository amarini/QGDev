 ...
 */
treeLooper::treeLooper(TString file, TString jetType, TString qgMiniTuplesDir = "~amarini/public/merged/QGMiniTupleSpring15/puppi"){
  ...
  if(usePtHatBins){
    ...
    for(TString ptHatBin : ptHatBins) 	qgMiniTuple->Add(qgMiniTuplesDir + "/QCD_Pt_"+ ptHatBin +"_TuneCUETP8M1_13TeV_pythia8.root", -1);
  } else if(file == "test"){	 	qgMiniTuple->Add("../test/qgMiniTuple.root", -1);
  } else 				qgMiniTuple->Add(qgMiniTuplesDir + "/" + file + ".root", -1);
