...
binClass get80XBinning(){
  binClass bins;
  bins.setBinRange("eta", 	"#eta",			{0,1.3,1.8,2.1,2.4,2.7,3,4.7});
  bins.setBinRange("pt" , 	"p_{T}",		bins.getBins(20, 20, 2000, true, {6500}));
  bins.setBinRange("rho",	"#rho",			{0,10,15,20,25,30,40,9999});								// Very preliminary binning for spring15dr asympt25ns rho distribution (peaks around ~12)
  bins.printBinRanges();

  // only high PT central, no rho
 for(int i=0; i < 5; ++i){	// No rho binning needed in the central (eta < 2.4) as rho dependencies are very small 
   for(int j=18-i; j < bins.getNBins("pt"); ++j){
     for(int k=1; k < bins.getNBins("rho"); ++k){
       bins.setLink(TString("eta") + i + "_pt" + j + "_rho0", TString("eta") + i + "_pt" + j + "_rho" + k);
     }
   }
